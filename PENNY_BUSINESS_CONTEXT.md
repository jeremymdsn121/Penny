# Penny — Business Context

A self-contained brief for discussing the business side of Penny with an AI assistant. Last updated 2026-06-01.

---

## What Penny is

**Penny** is a B2B SaaS virtual transaction coordinator (TC) for real estate brokerages. She's a conversational AI assistant — accessible over WhatsApp, SMS, and a web chat interface — that helps agents and brokers manage deals from contract to close.
...
- **Per-seat recurring subscription** + a small platform base fee.
  - Per-seat: justified by fairness and COGS (Penny's costs scale with usage — AI calls, WhatsApp, email — so seats proxy ability-to-pay)
  - Base fee: ensures even a 1–2 person brokerage generates a revenue floor in slow months
  - Recurring over per-transaction: Penny's value is "always-on standby"; per-transaction creates usage-anxiety and earns too little from seasonal brokerages
...
- Beachhead states: TX, FL, CA, NY, SC (states with deep compliance rulesets where Penny's AI review adds the most value)
...
- A display name ("Penny") approved separately by Meta
...
- Recipient **opt-in** required before Penny can initiate messages
...
**Architecture decision pending:** does the WhatsApp sender live under a single company WABA (Jeremy/Penny manages approval once) or does each brokerage register their own number? This choice affects onboarding scalability and who owns verification.

**Entity:** plan is to submit Meta Business Verification under Jeremy's existing registered LLC.

### 2. DNS — reply.poweredbypenny.com
`reply.poweredbypenny.com` needs an MX record pointing to `mx.sendgrid.net`. This is the gating step for inbound email threading (Section 4) and the reply-forwarding feature. DNS itself is quick to configure; propagation is the lag.
...
1. **State association forms** — Penny cannot generate or distribute NAR/state-promulgated contract forms without licensing agreements (6–18 month process). Penny can *extract data from* forms agents already have.
