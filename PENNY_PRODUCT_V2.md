...
Penny is a **per-brokerage assistant**, not a consumer app. Each brokerage gets
...
brokerage's agents work with Penny day to day.

The product thesis is unchanged from day one: a brokerage's transaction
coordinator role is expensive, hard to staff, and mostly pattern-driven. Penny
...
What has changed is the **shape** of the product. Penny began as a single-agent
...
- **One channel to omnichannel.** WhatsApp text was the entry point. Penny now
...
- **Reactive to proactive.** Penny used to act when asked. She now generates the
...
- **Individual tool to brokerage system.** Per-agent style, a broker review
  ...
  Penny from a personal assistant into something the brokerage runs on.
...
- **Agents.** Manage their active deals day to day. Most reach Penny over
  WhatsApp or SMS; the web app is there for heavier work and for the
  chat-forward "Ask Penny" experience.
- **Transaction coordinators.** Where a brokerage has them, Penny augments them
  and removes the rote chasing. Where it does not, Penny fills the gap.

---

## 3. How Agents Reach Penny (Channels)

Penny is deliberately channel-flexible, because agents live in their phones.

- **WhatsApp (text, voice, and media).** The primary channel. Agents text Penny,
...
- **Web chat ("Ask Penny").** The same assistant, exposed in the browser. It is
...
Penny's answer is **CSV import**. The broker exports their open deals, drops the
file in, and Penny maps the columns to its own fields. Because real exports never
use Penny's exact headers, the importer **aliases the common variants** ("Property
...
The single most important design decision in Penny is **where the human gate
...
e-signature envelope. On messaging Penny enforces the gate conversationally; in
...
routing, status updates, MLS prep). When a task is autonomous, Penny acts
...
brokerage can let Penny send routine intros on her own while still reviewing
everything else.

**One gate never lifts.** Compliance review can never be made autonomous. It is
locked off in policy and in code at every level. Penny always surfaces findings
to a human and records only the human's decision. This is a deliberate
load-bearing constraint, not a configuration default.

This model is what lets Penny scale from "drafts everything for review" to
...
- **Chat-forward home.** The landing screen leads with "Ask Penny": a greeting,
...
Penny's name is fixed product-wide and she is referred to as she/her in all
copy.
...
- **MLS publishing** — there is no universal MLS write API, so real publishing
  ...
  deferred part.
- **DocuSign** — e-signature send. Scoped narrowly on purpose: Penny sends
...
1. **Human-in-the-loop for anything external.** Penny drafts; humans approve.
...
6. **Stay in scope.** Penny is not a forms library, not accounting software, and
   not a system of record for legal advice. She coordinates; she does not
   replace the professionals.

---

## 11. Scope Boundaries (What Penny Deliberately Is Not)

Being explicit about the edges, because they are where outside critique is most
useful:

- **Not a forms library.** Penny sends and tracks documents she already has
  (extracted contracts, generated correspondence). Distributing blank state
  association forms requires licensing Penny does not hold.
...
- **Not a compliance authority.** Compliance review surfaces findings against a
  ...
  auto-approves.
- **Not an auto-responder.** Penny threads and logs inbound email and drafts
  replies on request, but never sends a reply without a human.
- **Not a live integration with other TC tools.** Penny imports existing deals
